...
class CapeSandboxConnector:
    def __init__(self):
        ...
        self._cooldown_time = int(get_config_variable(
        ...
        self.enforce_timeout = str(get_config_variable(
    ...
    def _send_knowledge(self, observable, report):
        ...
        for tactic_dict in report["ttps"]:
            attack_id = tactic_dict["ttps"][0]  # to be changed, ig used to send only 1 ttp, but noy many ttps for each tactic too ???
    ...
    def _trigger_sandbox(self, observable):
        ...
        # )
        response_dict={'error': False, 'data': {'task_ids': [413], 'message': 'Task ID 413 has been submitted'}, 'errors': [], 'url': ['http://example.tld/submit/status/413/']}
        print(response_dict)

        task_id = response_dict["data"]["task_ids"][0]
        if isinstance(task_id, list):
            task_id = response_dict["data"]["task_ids"][0][0]
        print(f"Analysis {task_id} has started...")
        ...
        while True:
            ...
            print(f'Analysis {task_id} has status "{status}"...')
            time.sleep(20)

        # Analysis is finished, process the report
        response_dict = self._get_report(task_id)

        print(f"Analysis {task_id} finished, processing report...")
